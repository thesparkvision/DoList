...
def create_task(
    mysql_session: Session, user_id: int, task_detail: task_schema.TaskRequestSchema
) -> Task:
    """
    Service method for creating a task.
    """

    new_task = Task(**task_detail.dict(), user_id=user_id)
    created_task: Task = sql_db.add_task(mysql_session, new_task)
    return created_task


def get_task(mysql_session: Session, user_id: int, task_id: int) -> Task:
    """
...
def get_all_tasks(mysql_session: Session, user_id: int) -> list[Task]:
    """ "
